...
app.post("/api/auth/login", async (req, res) => {
    ...
    try {
        ...
        const token = jwt.sign(
            { id: user.id, username: user.username, role: user.role, artistId: user.artist_id },
            JWT_SECRET,
            { expiresIn: "24h" }
        );
...
// Artist Routes
// ----------------------------------------------------
app.post("/api/artist/profile", authenticateToken, (req: any, res) => {
...
// ----------------------------------------------------
app.get("/api/payments/stripe/onboarding", authenticateToken, async (req: any, res) => {
    try {
        const userId = req.user.id;
...
// ----------------------------------------------------
app.get("/api/storage/gdrive/auth", authenticateToken, (req: any, res) => {
    const userId = req.user.id;
    const host = req.get("host");
    const publicUrl = `${req.protocol}://${host}`;
...
app.get("/api/storage/gdrive/files", authenticateToken, async (req: any, res) => {
    try {
...
// ----------------------------------------------------
app.post("/api/catalog/albums", authenticateToken, (req: any, res) => {
    const { title, slug, price, currency } = req.body;
    if (!title || !slug) return res.status(400).json({ error: "Title and slug are required" });

    try {
        const user = dbService.getUserById(req.user.id);
        if (!user.artist_id) return res.status(403).json({ error: "Artist profile required to create catalog items" });
...
app.post("/api/catalog/tracks", authenticateToken, (req: any, res) => {
...
app.get("/api/catalog/albums", (req, res) => {
    const artistId = req.query.artistId ? parseInt(req.query.artistId as string, 10) : null;
