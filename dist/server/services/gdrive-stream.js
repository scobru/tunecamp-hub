...
export class GoogleDriveService {
    ...
    async getValidToken(userId) {
        ...
        const expiryDate = Date.now() + expires_in * 1000;
        this.db.upsertStorageAccount(userId, "google", account.account_email || "", access_token, account.refresh_token, expiryDate);
        return access_token;
    }
    ...
    async listAudioFilesRecursive(userId, folderId = "root") {
        const audioFiles = [];
    ...
    async getFileStream(userId, fileId, range) {
        const token = await this.getValidToken(userId);
...
//# sourceMappingURL=gdrive-stream.js.map